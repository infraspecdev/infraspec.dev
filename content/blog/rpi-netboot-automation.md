...
The `pxeService.sh` script is designed to automate the setup for each PXE client (Raspberry Pi device). You can look at the complete `pxeService.sh` script [here](https://github.com/infraspecdev/rpi-network-boot/blob/main/pxeScript.sh). Here's a breakdown of the script:
